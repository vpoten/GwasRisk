 ...
 * ------------------------ */
   ...
   public void recreate(int m, int n) {
      this.m = m;
      this.n = n;
      A = new byte[m][n];
   }
   ...
   */
   ...
   public byte[] getColumnPackedCopy () {
      byte[] vals = new byte[m*n];
      for (int i = 0; i < m; i++) {
         for (int j = 0; j < n; j++) {
            vals[i+j*m] = A[i][j];
         }
...
//   }
   ...
   */
   ...
   public Matrix timesEquals (byte s) {
      for (int i = 0; i < m; i++) {
         for (int j = 0; j < n; j++) {
            A[i][j] = (byte) (s*A[i][j]);
         }
...
//   }
   ...
   */
   ...
   public static Matrix random (int m, int n) {
      ...
      for (int i = 0; i < m; i++) {
         for (int j = 0; j < n; j++) {
            X[i][j] = (byte) (Math.random()*Byte.MAX_VALUE);
         }
      }
      return A;
   }
   ...
   */
   ...
   public static Matrix read (BufferedReader input) throws java.io.IOException {
	...
	throw new java.io.IOException("Unexpected EOF on matrix read.");
      ...
      byte[][] A = new byte[m][];
      v.toArray(A);  // copy the rows out of the vector
      return new Matrix(A);
   }
