...
if( !this.args || this.args.length!=3 ){
    ...
    println 'usage: genotypes_to_tped.groovy <input_dir> <output_dir> <tfam_base_file>'
    return 1
}
...
CHR_LIST.each{ chr->
    ...
    //collect snps
    subjects.each{ subj->
        def file = genotypeFiles.find{ def mat = (it =~ regexGenoTxt)[0];
            ((mat[1]==subj) && (mat[2]==chr)) }
        
        def reader = createReader(new File(input+file))
