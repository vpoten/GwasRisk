...
def genOutFileName = { disease, nbest, set, type ->
    "${disease}_size1_best${nbest}_${set}.${type}.arff"
}//
...
/**
 * closure that compares two snps 'snp@pval'
 */
def snpComparator = [
    compare:{a,b->
        def pvalA = a.substring(a.indexOf('@')+1) as Double
        def pvalB = b.substring(b.indexOf('@')+1) as Double
        return pvalA<=>pvalB
    }
] as Comparator
...
def getBestSNP = { input, nbest ->
        ...
        reader.eachLine{ line ->
            if( line ==~ filterReg ){
                ...
                try{
                    ...
                    bestSnps << "${toks[snpIdx]}@${pval}"
                } catch(NumberFormatException e){}
                
                if( bestSnps.size()>nbest ){
                    bestSnps.pollLast()
                }
            }
        }
    ...
    def selected = bestSnps.collect{ it.substring(0, it.indexOf('@')) } as TreeSet
...
if( !this.args || this.args.length!=2 ){
    ...
    println 'usage: ped_to_arff3.groovy <output_dir> <best_count>'
    return 1
}

String outdir = this.args[0]
int numBest = this.args[1] as Integer
...
println "Best SNPs count: ${numBest}"
...
DISEASES.each{ dis->
    ...
    println "*** get best snps #${numBest}:"
    def chrSnps = getBestSNP(disOutput, numBest)
    ...
    if( numSnps>0 ){
        ...
        genotypes.each{ set, map->
            ['rs','grs','wgrs'].each{ type->
                def outFile = disOutput+genOutFileName(dis, numBest, set, type)
